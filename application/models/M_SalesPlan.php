...
class M_SalesPlan extends CI_Model{

	function cek_salesplan($kode,$kode_manager){
		$sql = "SELECT * FROM dbo.ProdukSalesPlan
	...
	function get_produkflag($kode,$kode_manager){
		...
		if ($kode==1) {
			$sql = "SELECT ps.Kode, NamaProduk, Kemasan, NamaGrp, NAMA AS NamaSupplier, NamaManager AS SM, 
					...
					FROM dbo.Produk p
					JOIN dbo.ProdukSalesPlan ps ON p.KodeProduk=ps.KodeProduk AND KodeGrp=KodeGroup
					...
					AND Periode='$xPeriode' AND Tahun='$xTahun'";
			return $this->db->query($sql);
		}elseif ($kode==2) {
			$sql = "SELECT ps.Kode, NamaProduk, Kemasan, NamaGrp, NAMA AS NamaSupplier, NamaManager AS SM, 
					CAST(ps.SalesPlan AS INT) AS SalesPlan, SalesPlanKtn 
			...
			$sql = "SELECT  ps.Kode, NamaProduk, Kemasan, NamaGrp, NAMA AS NamaSupplier 
					...
					JOIN dbo.ProdukSalesPlan ps ON p.KodeProduk=ps.KodeProduk AND KodeGrp=KodeGroup
					JOIN dbo.jass_supplier2 s ON KodeSupplier=s.KODE
					WHERE ps.ProdukFlag='$kode' AND ps.KodeCabang LIKE '$kode_manager%' AND LEFT(KodeSupplier,5)=left('$kode_manager',5) AND 
					(CASE WHEN KodeGrp='SGLB' AND KodeSupplier='00001/0000005' THEN NULL ELSE KodeSupplier END) IS NOT NULL";
			return $this->db->query($sql);
		}elseif ($kode==4) {
		...
		if ($produk_flag==1) {
			...
			// 		AND ps.Kode='$kode'";
				$sql = "SELECT ps.Kode, p.KodeProduk, p.NamaProduk +' ('+ Kemasan +')' AS NamaProduk, NamaGrp, NAMA AS NamaSupplier, NamaManager AS SM, 
						CAST(ps.SalesPlan AS INT) AS SalesPlan, SalesPlanKtn, (SELECT dbo.f_AmbilWeekPeriode(DATEADD(dd,-(DAY(DATEADD(mm,1,DATEADD(m,1,GETDATE())))-1),DATEADD(mm,0,DATEADD(m,1,GETDATE()))),'''')) * SUM(rpp.RPPQty) AS RppBulan, ISNULL(l.Limit,0) AS Limit 
							FROM dbo.Produk p
							JOIN dbo.ProdukSalesPlan ps ON p.KodeProduk=ps.KodeProduk AND KodeGrp=KodeGroup
							JOIN dbo.jass_supplier2 s ON KodeSupplier=s.KODE
							JOIN (SELECT DISTINCT KodeManager, NamaManager FROM dbo.Salesman) m ON ps.KodeManager=m.KodeManager
	...
	function getAllProdukWithFlag($kodeSupplier){
		...
		return $this->db
			->select('
				...
				dbo.f_AmbilRppBulan(rpp.KODESM, pr.KodeSupplier, pr.KodeGrp, pr.KodeProduk) AS RppBulan,
				ISNULL(sp.SalesPlanKtn,dbo.f_AmbilRppBulan(rpp.KODESM, pr.KodeSupplier, pr.KodeGrp, pr.KodeProduk)) SalesPlanKtn,
				sp.SalesPlan,
				ISNULL(l.Limit,0) AS Limit
				')
			->from('Produk pr')
			->join('(SELECT * FROM dbo.RppRegSM WHERE KodeSM in \''.$kode_manager.'\') Rpp','pr.KodeProduk=Rpp.KODEPRODUK')
			->join('ProdukSalesPlan sp','pr.KodeProduk = sp.KodeProduk AND Rpp.KODESM = sp.KodeManager AND sp.Tahun = '.$xTahun.' AND sp.Periode = '.$xPeriode,'left')
	...
	function get_produk($depo,$supplier,$group){
		$kode_manager=$this->session->userdata('KodeUser');
		$kode_altius=$this->session->userdata('KodeAltius');
		$xPeriode=$this->session->userdata('Periode');
		$xTahun=$this->session->userdata('Tahun');
		$sql = "SELECT DISTINCT Produk.KodeProduk, Produk.NamaProduk +' ('+ Kemasan +')'  as NamaProduk 
				FROM 	Produk
				JOIN (SELECT * FROM dbo.RppRegSM WHERE KodeSM IN $kode_manager) AS Rpp ON Produk.KodeProduk=Rpp.KODEPRODUK  
				WHERE Produk.KodeProduk NOT IN (SELECT DISTINCT KodeProduk FROM dbo.ProdukSalesPlan WHERE (KodeManager IN $kode_manager OR KodeCabang=LEFT('$kode_altius',8)) AND Periode=$xPeriode AND Tahun=$xTahun ) AND Produk.NamaProduk NOT LIKE '%***%' AND
	...
	function get_produk2($depo,$supplier,$group){
		...
		$xPeriode=$this->session->userdata('Periode');
		$xTahun=$this->session->userdata('Tahun');
		$sql = "SELECT p.KodeProduk, p.NamaProduk +' ('+ Kemasan +')'  as NamaProduk,(SELECT dbo.f_AmbilWeekPeriode(DATEADD(dd,-(DAY(DATEADD(mm,1,DATEADD(m,1,GETDATE())))-1),DATEADD(mm,0,DATEADD(m,1,GETDATE()))),'''')) * SUM(rpp.RPPQty) AS RppBulan, ISNULL(l.Limit,0) AS Limit
				FROM 	Produk p
				...
				LEFT JOIN dbo.LimitSpProduk l ON p.KodeSupplier=l.KodeSupplier AND p.KodeGrp=l.KodeGroup AND p.KodeProduk=l.KodeProduk
				WHERE p.KodeProduk NOT IN (SELECT DISTINCT KodeProduk FROM dbo.ProdukSalesPlan WHERE (KodeManager IN $kode_manager OR KodeCabang=LEFT('$kode_altius',8)) AND Periode=$xPeriode AND Tahun=$xTahun AND ProdukFlag IN (1,4)) AND
				p.KodeProduk NOT IN (SELECT DISTINCT KodeProduk FROM dbo.ProdukSalesPlan WHERE (KodeManager IN $kode_manager OR KodeCabang=LEFT('$kode_altius',8)) AND ProdukFlag IN (2,3)) AND p.NamaProduk NOT LIKE '%***%' AND
				p.KodeSupplier like '$depo'+'/$supplier%' AND p.KodeGrp LIKE '$group%' 
				GROUP BY p.KodeProduk, p.NamaProduk, p.Kemasan, l.Limit";
				// AND dbo.f_AmbilRppBulan(KODESM, p.KodeSupplier, p.KodeGrp,p.KodeProduk) > 0";
		return $this->db->query($sql);
	}
	...
	function get_group($depo,$supplier){
		...
		$sql = "SELECT Distinct KodeGrp, NamaGrp FROM Produk 
				JOIN (SELECT DISTINCT KodeSupplier FROM dbo.RppRegSM WHERE KodeSM IN $kode_manager) AS Rpp ON Produk.KodeSupplier=Rpp.KodeSupplier
	...
	function get_rpp_bulan($supplier,$supplier2,$group,$produk){
		$sql = "SELECT dbo.f_AmbilRppBulan('".$this->session->userdata('KodeManager')."','$supplier'+'/$supplier2','$group','$produk') AS RppBulan";
		return $this->db->query($sql);
	}
	...
	function add_salesplan2($kode,$produk_flag,$kode_supplier,$kode_produk,$kode_group,$kode_cabang,$kode_manager,$sales_plan,$sales_plan_ktn){
		$xPeriode=$this->session->userdata('Periode');
		$xTahun=$this->session->userdata('Tahun');
		$sql = "EXEC dbo.Add_SalesPlan 
				@Kode = '$kode',  @ProdukFlag = $produk_flag, @KodeSupplier='$kode_supplier',  @KodeProduk = '$kode_produk',  @KodeGroup = '$kode_group',  
				@KodeCabang = '$kode_cabang',  @KodeManager = '$kode_manager',  @SalesPlan = $sales_plan,  @SalesPlanKtn = $sales_plan_ktn, 
				@Periode = $xPeriode, @Tahun = $xTahun ";
		$this->db->query($sql);
	}
	...
	{
		$this->db->where($where);
		$this->db->delete('ProdukSalesPlan');
	}

	function insertSalesPlanKlasifikasi($data)
	{
		$this->db->insert('ProdukSalesPlan',$data);
	}
	...
	function delete_salesplan($produk_flag,$kode){
		...
		$xTahun=$this->session->userdata('Tahun');
		if ($produk_flag==1) {
			$sql = "DELETE FROM dbo.ProdukSalesPlan WHERE ProdukFlag='$produk_flag' AND Kode='$kode' AND KodeCabang = '$kode_cabang'
					AND Periode=$xPeriode AND Tahun=$xTahun";
			$this->db->query($sql);
	...
	function copy_salesplan($produk_flag,$kode_manager,$periode,$tahun,$periode_now,$tahun_now){
		if ($produk_flag==1) {
