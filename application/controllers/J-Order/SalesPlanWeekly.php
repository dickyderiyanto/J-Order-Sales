...
class SalesPlanWeekly extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('M_SalesPlanWeekly');
   	}
	...
	public function index()
	{
		...
		$data['ProdukFlag'] = $this->M_SalesPlanWeekly->get_produkflag('1',$KodeManager)->result();
		// $data['Produk'] = $this->M_SalesPlanWeekly->get_produk()->result();
		// $data['Group'] = $this->M_SalesPlanWeekly->get_group()->result();
		$data['Supplier'] = $this->M_SalesPlanWeekly->get_supplier()->result();

		$this->load->view('v_header',$data);
		$this->load->view('J-Order/v_menu');
		$this->load->view('J-Order/v_product_focus_weekly',$data);
	...
	public function add_klasifikasi()
	{
		...
		if($this->input->method(TRUE) == "POST") {
			...
			$this->M_SalesPlanWeekly->deleteSalesplanWhere(array(
			...
			if($ProdukFlag != 5){
				$this->M_SalesPlanWeekly->insertSalesPlanKlasifikasi(array(
	...
	public function add_focus()
	{
        ...
        else
        {
        	...
        	$focus = $this->M_SalesPlanWeekly->get_produk3($KodeSupplier,$KodeGroup);
			if($focus->num_rows() > 0){
				foreach ($focus->result() as $data_focus){
		        	...
		        	if ($SalesPlan != '' and $SalesPlan != 0) {
		        		$this->M_SalesPlanWeekly->add_salesplan2($Kode,1,$KodeSupplier,$KodeProduk,$KodeGroup,$KodeCabang,$KodeManager,$SalesPlan,$SalesPlanKtn);	
		        	}
				}
			}
			...
			// $cek = $this->M_SalesPlanWeekly->cek_salesplan($Kode,$KodeManager);
			// if($cek->num_rows() > 0){
			// 	$this->session->set_flashdata('pesan', 'Data sudah ada');
   //        		redirect(base_url('J-Order/SalesPlan'));
			// }
			// else{
			// 	$this->M_SalesPlanWeekly->add_salesplan($data,'ProdukSalesPlan');
			// 	$data['message'] = 'Data Inserted Successfully';
				redirect(base_url('J-Order/SalesPlanWeekly'));
			// }
		...
		public function Declaining()
	{
		...
		$data['ProdukFlag'] = $this->M_SalesPlanWeekly->get_produkflag(2, $KodeManager)->result();
		...
		$this->load->view('v_header',$data);
		$this->load->view('J-Order/v_menu');
		$this->load->view('J-Order/v_declaining',$data);
		$this->load->view('v_footer');
			...
			if($focus->num_rows() > 0){
				foreach ($focus->result() as $declaining){
					...
					if (!empty($SalesPlanKtn) && $SalesPlanKtn > 0) {
						$this->M_SalesPlanWeekly->add_salesplan2($Kode,2,$KodeSupplier,$KodeProduk,$KodeGroup,$KodeCabang,$KodeManager,0,$SalesPlanKtn);	
					}
				}
			}

			redirect(base_url('J-Order/SalesPlanWeekly/Declaining'));
		}
	}
	...
	public function Discontinued()
	{
		...
		$data['ProdukFlag'] = $this->M_SalesPlanWeekly->get_produkflag(3,$KodeDepo)->result();
		// $data['Produk'] = $this->M_SalesPlan->get_produk()->result();
		// $data['Group'] = $this->M_SalesPlan->get_group()->result();
		$data['Supplier'] = $this->M_SalesPlanWeekly->get_supplier()->result();
		...
		$this->load->view('v_header',$data);
		$this->load->view('J-Order/v_menu');
		$this->load->view('J-Order/v_discontinued_weekly',$data);
		$this->load->view('v_footer');
		// echo base_url('assets/css/bootstrap.min.css');
	}
	...
	public function add_discontinued()
	{
		...
		else
		{
			...
			$KodeCabang = $this->session->userdata('KodeCabang');
			$KodeManager = $this->session->userdata('KodeManager');
			$Pweek = $this->session->userdata('PeriodeWeek');
			$Tahun=$this->session->userdata('Tahun');

			if($KodeProduk == 'All'){
				$SuppArr = explode('/', $KodeSupplier);
				$Produk = $this->M_SalesPlanWeekly->get_produk($SuppArr[0],$SuppArr[1],$KodeGroup);
				foreach ($Produk->result() as $row) {
					...
					$cek = $this->M_SalesPlanWeekly->cek_salesplan($Kode,$KodeCabang);
					if($cek->num_rows() > 0) continue;
					$this->M_SalesPlanWeekly->add_salesplan($data,'ProdukSalesPlanWeekly');
				}
				$data['message'] = 'Data Inserted Successfully';
				redirect(base_url('J-Order/SalesPlanWeekly/Discontinued'));
			} else{
	...
	public function Seasonal()
	{
		...
		$data['ProdukFlag'] = $this->M_SalesPlanWeekly->get_produkflag('4',$KodeManager)->result();
		// $data['Produk'] = $this->M_SalesPlan->get_produk()->result();
		// $data['Group'] = $this->M_SalesPlan->get_group()->result();
		$data['Supplier'] = $this->M_SalesPlanWeekly->get_supplier()->result();
			...
			if($focus->num_rows() > 0){
				foreach ($focus->result() as $data_focus){
					...
					if ($SalesPlanKtn != '' and $SalesPlanKtn != 0) {
						$this->M_SalesPlanWeekly->add_salesplan2($Kode,4,$KodeSupplier,$KodeProduk,$KodeGroup,$KodeCabang,$KodeManager,0,$SalesPlanKtn);	
					}
				}
			}
			redirect(base_url('J-Order/SalesPlanWeekly/Seasonal'));
		}
	}

	public function GetJson($produk_flag,$kode) {
		$KodeManager=$this->session->userdata('KodeManager');
		$result = $this->M_SalesPlanWeekly->get_produkflag2($produk_flag,$kode,$KodeManager)->result();
	...
	public function ajax_supplier() {
		$result = $this->M_SalesPlanWeekly->get_supplier()->result();
		foreach ($result as $data) {
			$arr[]=$data;
		}
		echo json_encode($arr);
	...
	public function ajax_group($depo,$supplier) {
		$result= $this->M_SalesPlanWeekly->get_group($depo,$supplier)->result();
		foreach ($result as $data) {
			$arr[]=$data;
		}
		echo json_encode($arr);
	...
	public function ajax_produk($depo,$supplier,$group) {
		$result= $this->M_SalesPlanWeekly->get_produk($depo,$supplier,$group)->result();
		foreach ($result as $data) {
			$arr[]=$data;
		}
		echo json_encode($arr);
	...
	public function ajax_produk2($depo,$supplier,$group) {
		$result= $this->M_SalesPlanWeekly->get_produk2($depo,$supplier,$group)->result();
		foreach ($result as $data) {
			$arr[]=$data;
		}
		echo json_encode($arr);
	...
	public function ajax_rpp_bulan($supplier,$supplier2,$group,$produk) {
		$result= $this->M_SalesPlanWeekly->get_rpp_bulan($supplier,$supplier2,$group,$produk)->row();
		echo json_encode($result);
	}

	public function ajax_limit_produk($supplier,$supplier2,$group,$produk) {
		$result= $this->M_SalesPlanWeekly->get_limit_produk($supplier,$supplier2,$group,$produk)->row();
		echo json_encode($result);
	}

	public function hapus($produk_flag,$kode) {
		if ($produk_flag==1) {
			$this->M_SalesPlanWeekly->delete_salesplan_weekly($produk_flag,$kode);
			redirect(base_url('J-Order/SalesPlanWeekly'));
		}elseif ($produk_flag==2) {
			$this->M_SalesPlanWeekly->delete_salesplan($produk_flag,$kode);
			redirect(base_url('J-Order/SalesPlanWeekly/Declaining'));
	...
	public function copy_data($produk_flag) {
		...
		if ($produk_flag==1) {
			$this->M_SalesPlanWeekly->copy_salesplan($produk_flag,$xKodeManager,$xTahun,$xPeriodeNow,$xTahunNow);
			redirect(base_url('J-Order/SalesPlanWeekly'));
   		// }elseif ($produk_flag==2) {
   		// 	# code...
   		// }elseif ($produk_flag==3) {
	...
	public function Klasifikasi()
	{
		...
		$this->load->view('v_header',$data);
		$this->load->view('J-Order/v_menu');
		$this->load->view('J-Order/v_klasifikasi',$data);
		$this->load->view('v_footer');
		// echo base_url('assets/css/bootstrap.min.css');
	}
	...
	public function ajax_principle() {
		...
		$data = $this->M_SalesPlanWeekly->get_supplier($kode_cabang)->result();
		echo json_encode($data);
	}
	...
	public function ajax_klasifikasi()
	{
		...
		$data = $this->M_SalesPlanWeekly->getAllProdukWithFlag($KodeSupplier)->result();
		echo json_encode($data);
	}
