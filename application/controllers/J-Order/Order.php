...
class Order extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('J-Order/v_order',$data);
	...
	// 	$data = $this->M_Order->get_order($kode_cabang,$kode_supplier)->result();
 ...
 //   public function ajax_supplier() {
   ...
   public function ajax_order($kode,$supplier) {
		...
		$xTahun=$this->session->userdata('TahunWeek');
		$kode_supplier=$kode."/".$supplier;
		$data = $this->M_Order->get_order($kode_cabang,$kode_supplier,$xPweek,$xTahun,$xIdUser)->result();
       	// $result = $this->M_Order->get_order(0)->result();
		echo json_encode($data);
   }

   public function ajax_jadwaldo($kode,$supplier) {
		$kode_supplier=$kode."/".$supplier;
		$data = $this->M_Order->get_jadwaldo($kode_supplier)->result();
       	// $result = $this->M_Order->get_order(0)->result();
		echo json_encode($data);
   }
   ...
   public function cetak($id,$id2)
	  {
	    ...
	    $temp_rec = $this->M_Order->cetak_order($kode_cabang,$xPrinciple,$xPweek,$xTahun);
	...
	public function export_excel($id,$id2){
	  ...
	  // 	}else{
		...
		$order = $this->M_Order->struk_order($kode_cabang,$xPrinciple,$xPweek,$xTahun);
		...
		$no_order = $this->NoOrder($xPweek, $xPrinciple, date('m'), $xTahun);
		$tgl_order = date('Y-m-d');
		$nama_principle = $this->M_Order->principle($xPrinciple)->row()->Nama;
	    ...
	    $excel->setActiveSheetIndex(0)->setCellValue('C8', ": ".$this->M_Order->tgl_indo(date('Y-m-d',strtotime($tgl_order)),'d'));
		...
		$query = $this->M_Order->cetak_order($kode_cabang,$xPrinciple,$xPweek,$xTahun);
	  ...
	  private function NoOrder($week, $kode, $month, $year)
	  {
	  	if($week < 10)
	  		$xweek = '00'.$week;
	  	else
	  	...
	  	return $xweek.'/JP/'.substr($kode,6).'/'.$this->GetRomawi($month).'/'.$year;
	  }

	  private function GetRomawi($number)
	  {
